...
class LocalSerializerTest : public ::testing::Test {
 public:
  ...
  static void ExpectSet(_google_firestore_v1_Write encoded) {
    ...
    EXPECT_EQ("b", nanopb::MakeString(encoded.update.fields[0].value.string_value));
  ...
  static void ExpectUpdateTransform(_google_firestore_v1_Write encoded) {
    ...
    EXPECT_EQ(google_firestore_v1_DocumentTransform_FieldTransform_increment_tag, encoded.update_transforms[0].which_transform_type);
    EXPECT_EQ("integer", nanopb::MakeString(encoded.update_transforms[0].field_path));
    EXPECT_EQ(42, encoded.update_transforms[0].increment.integer_value);
    EXPECT_EQ(google_firestore_v1_DocumentTransform_FieldTransform_increment_tag, encoded.update_transforms[1].which_transform_type);
    EXPECT_EQ("double", nanopb::MakeString(encoded.update_transforms[1].field_path));
...
TEST_F(LocalSerializerTest, MultipleMutationsAreSquashed) {
  ...
  *batch_proto.mutable_local_write_time() = WriteTimeProto();
  ...
  _google_firestore_v1_Write encoded = remote_serializer.EncodeMutation(decoded.mutations()[0]);
  ExpectSet(encoded);
